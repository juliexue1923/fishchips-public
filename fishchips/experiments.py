...
class CMB_Primary(Experiment):
    ...
    def get_fisher(self, obs, lensed_Cl=True):
        ...
        fid_pp = fid_cl['pp']
        fid_kk_list = []
        for x, y in zip(fid_ll, fid_pp):
            fid_kk_list.append(1/4*y*(x-1)*x*(x+1)*(x+2))
        fid_kk = np.array(fid_kk_list)
        fid_kk = fid_kk.astype('float64')
        fid = {'tt': (Tcmb*1.0e6)**2 * fid_cl['tt'],
               'te': (Tcmb*1.0e6)**2 * fid_cl['te'],
               'ee': (Tcmb*1.0e6)**2 * fid_cl['ee'],
               'kk': (Tcmb*1.0e6)**2 * fid_kk}
    ...
    def compute_fisher_per_l(self, fid, df, pars):
        """
        Compute the Fisher element per l given fiducial and derivative dicts.

        """
        npar = len(pars)
        # following eq 4 of https://arxiv.org/pdf/1402.4108.pdf
        fisher_ij_l = np.zeros(self.l_max)
        # probably a more efficient way to do this exists
        for l in range(self.l_min, self.l_max):
            Cl = np.array([[fid['tt'][l] + self.noise_T[l], fid['te'][l] + self.noise_TE[l]],
                               [fid['te'][l] + self.noise_TE[l], fid['ee'][l] + self.noise_P[l]]])
            ...
            dCl = np.array([[df[pars[0]+'_tt'][l], df[pars[0]+'_te'][l]],
                                  [df[pars[0]+'_te'][l], df[pars[0]+'_ee'][l]]])

            inner_term = np.dot(np.dot(invCl, dCl), np.dot(invCl, dCl))
            fisher_contrib = (2*l+1)/2. * self.f_sky * np.trace(inner_term)
            fisher_ij_l[l]= fisher_contrib

        return fisher_ij_l
    
    def compute_fisher_per_l_TT(self, fid, df, pars):
        """
        Compute the Fisher element per l given fiducial and derivative dicts, using only TT information.
        ...
        # probably a more efficient way to do this exists
        for l in range(self.l_min, self.l_max):
            Cl = np.array([[fid['tt'][l] + self.noise_T[l]]])
            invCl = np.linalg.inv(Cl)
            dCl = np.array([[df[pars[0]+'_tt'][l]]])
            inner_term = np.dot(np.dot(invCl, dCl), np.dot(invCl, dCl))
            fisher_contrib = (2*l+1)/2. * self.f_sky * np.trace(inner_term)
            fisher_ij_l[l]= fisher_contrib
    ...
    def compute_fisher_per_l_kk(self, fid, df, pars, kknoise=[]):
        """
        Compute the Fisher element per l given fiducial and derivative dicts, using both primary spectra and an extra array of lensing noise information.
        ...
        # probably a more efficient way to do this exists
        for l in range(self.l_min, self.l_max):
            Cl = np.array([[fid['tt'][l] + self.noise_T[l], fid['te'][l] + self.noise_TE[l], 0],
                               [fid['te'][l] + self.noise_TE[l], fid['ee'][l] + self.noise_P[l], 0],
                                  [0, 0, fid['kk'][l] + kknoise[l]]])
            invCl = np.linalg.inv(Cl)

            dCl = np.array([[df[pars[0]+'_tt'][l], df[pars[0]+'_te'][l], 0],
                               [df[pars[0]+'_te'][l], df[pars[0]+'_ee'][l], 0],
                                   [0, 0, df[pars[0]+'_kk'][l]]])
    ...
    def get_fisher_per_l(self, obs, lensed_Cl=True):
        """
        Return Fisher elements per l.
        """
        # first compute the fiducial
        fid_cosmo = obs.cosmos['fiducial']
        Tcmb = fid_cosmo.T_cmb()
        ...
        return self.compute_fisher_per_l(fid,
                                         df,
                                         obs.parameters)
    
    def get_fisher_per_l_TT(self, obs, lensed_Cl=True):
        """
        Return Fisher elements per l, using only TT information (calling the TT function).
        """
        # first compute the fiducial
        fid_cosmo = obs.cosmos['fiducial']
        Tcmb = fid_cosmo.T_cmb()
        ...
        return self.compute_fisher_per_l_TT(fid,
                                         df,
                                         obs.parameters)
    
    def get_fisher_per_l_kk(self, obs, lensed_Cl=True, kknoise=[]):
        """
        Return Fisher elements per l,  using both primary spectra and lensing noise information.
        """
        # first compute the fiducial
        fid_cosmo = obs.cosmos['fiducial']
        Tcmb = fid_cosmo.T_cmb()
        ...
        for par, dx in zip(obs.parameters, dx_array):
            ...
            left_kk_list = []
            right_kk_list = []
            for x, y in zip(fid_ll, cl_left['pp']):
                left_kk_list.append(1/4*y*(x-1)*x*(x+1)*(x+2))
            for x, y in zip(fid_ll, cl_right['pp']):
                right_kk_list.append(1/4*y*(x-1)*x*(x+1)*(x+2))
            cl_left['kk'] = np.array(left_kk_list)
            cl_right['kk'] = np.array(right_kk_list)
            cl_left['kk'] = cl_left['kk'].astype('float64')
            cl_right['kk'] = cl_right['kk'].astype('float64')
            df[par + '_kk'] = (Tcmb*1.0e6)**2 *\
                    (cl_right['kk'] - cl_left['kk']) / dx
            
        return self.compute_fisher_per_l_kk(fid,
                                         df,
                                         obs.parameters, kknoise)
